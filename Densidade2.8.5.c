...
// Alteração em 05/2020
// Implementação do input por arquivo de setup

bool SETUPINPUT = false;

/////////// Defining Tuple struct ///////////////

static struct { char strVal[MAXCHAR]; int intVal; } tuple[MAXCHAR];
static int tupleCount = 0;
static void listTuples(void) {
	int a;
    printf("==========\nTuple count is %d\n", tupleCount);
    for (a = 0; a < tupleCount; ++a)
        printf("   [%s] -> %d\n", tuple[a].strVal, tuple[a].intVal);
    puts("==========");
}
...
int main(void)//PROGRAMA PRINCIPAL, O QUAL CHAMA TODAS AS FUNÇÕES///////////////
{
   ...
   FILE *fp; // Setup file
   char str_read[MAXCHAR]; // Variable to store the lines of Setup file
   char str[MAXCHAR];
   char* filename = "setup-file.txt"; // Setup file name
   ...
   if(fp != NULL){
      ...
      fclose(fp);
   	...
   	while(num != 2){     
	      //system("cls"); //COMANDO QUE LIMPA A TELA - BUGANDO//
	      printf("--------------------------------------------------------------------------------");
	      printf("                    DENSIDADE 2.8.5 - Qualidade de Vida (DEBUG MODE = ON)\n"); 
	      printf("--------------------------------------------------------------------------------\n\n");
	      printf("                        1---Calculos\n"); 
	      printf("                        2---Sair\n\n\n\n"); 
	      printf("Opcao ---> ");
		...
		printf("flag");
         ...
         while (tst[1] == '*')
         {
            ...
            La:
             ...
             if(SETUPINPUT == true){
             	memcpy(nome_arq, tuple[5].strVal, 9);
				nome_arq[9] = '\0';
		    }
		    else scanf("%s",&nome_arq);
...
///Desvio padrão
        ...
        while ((VolAns != 1) && (VolAns != 2))
        {
         ...
         if(SETUPINPUT == true){
         	memcpy(nome_arq_2, tuple[16].strVal, 4);
			nome_arq_2[4] = '\0';
		 }
		 else scanf("%s",&nome_arq_2);
         fflush(stdin);
         printf("\nDigite o nome do arquivo dos FATORES DE RESPOSTA de linha K: "); 
         if(SETUPINPUT == true){
